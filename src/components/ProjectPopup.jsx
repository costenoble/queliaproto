...
import { Building, ExternalLink, Home, Mic, Loader2, Car } from 'lucide-react';
  ...
  return (
    <div className="w-80 bg-white overflow-hidden font-sans rounded-lg shadow-sm">
      ...
      <div className="p-5">
        ...
        <span
          className="inline-flex items-center text-xs font-bold px-2 py-1 rounded uppercase tracking-wider text-white shadow-sm mb-2"
        ...
        {/* Ville / Agglomération (gauche) + Logo (droite) */}
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="min-w-0">
            <div className="flex items-center text-sm text-gray-600">
              <Building className="w-3.5 h-3.5 mr-1.5 text-gray-400 flex-shrink-0" />
              <span className="truncate">{locationParts.join(' • ') || 'Localisation non spécifiée'}</span>
            </div>
        ...
        {/* Contact + Propulsé par Quelia */}
        <div className="border-t border-gray-100 pt-3 mt-2 space-y-2.5">
          <a
            href="mailto:contact@quelia.fr"
            className="flex items-start text-sm text-gray-600 hover:text-indigo-600 transition-colors"
          >
            <Mic className="w-4 h-4 mr-2 text-gray-400 flex-shrink-0 mt-0.5" />
            <span>Signalez-nous toute observation sur le parc</span>
          </a>
          <a
